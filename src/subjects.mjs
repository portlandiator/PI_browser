...
function go(changes){history.pushState({},'',url(changes));return render();}
function directory(){
  const q=$('subject-query').value.toLowerCase(),thematic=params().get('order')!=='alphabetical';
  $('subject-order').value=thematic?'thematic':'alphabetical';
  const link=s=>`<a class="subject-label" style="${colorStyle(s)}" ${s.id===params().get('subject')?'aria-current="true"':''} href="${esc(url({subject:s.id,page:null,author:null,volume:null,availability:null}))}">${esc(s.name)} <small>(${s.selections??0})</small></a>`;
  let count=0;
  if(thematic){
    const byName=new Map(index.subjects.map(s=>[s.name,s]));
    $('subject-list').innerHTML=thematicGroups.map(group=>{
      const rows=group.subjects.map(name=>byName.get(name)).filter(s=>s&&(s.name.toLowerCase().includes(q)||group.title.toLowerCase().includes(q)));
      count+=rows.length;if(!rows.length)return '';
      return `<section class="subject-section" aria-labelledby="section-${esc(group.id)}"><h3 id="section-${esc(group.id)}">${esc(group.title)}</h3>${rows.map(link).join('')}</section>`;
    }).join('');
  }else{
    const rows=index.subjects.filter(s=>s.name.toLowerCase().includes(q)).sort((a,b)=>a.name.localeCompare(b.name));
    count=rows.length;$('subject-list').innerHTML=rows.map(link).join('');
  }
  $('subject-count').textContent=`${count} of ${index.subjects.length} subjects`;
}
...
const graphMeasure=document.createElement('canvas').getContext('2d');
function graphLines(name){
  graphMeasure.font='14px "EB Garamond"';
  const lines=[];let line='';
  for(const word of name.split(/\s+/)){
    const next=line?line+' '+word:word;
    if(graphMeasure.measureText(next).width<=122){line=next;continue;}
    if(line)lines.push(line);line='';
    for(const letter of word){if(line&&graphMeasure.measureText(line+letter).width>122){lines.push(line);line='';}line+=letter;}
  }
...
function neighborhood(subject){return graphNeighborhood(index.subjects,index.edges,subject.id,params().get('hops')||1,params().get('suggestions')!=='0');}
...
async function render(){
  ...
  const token=++epoch,activeId=document.activeElement?.id;try{if(!params().get('subject')){const initial=index.subjects.find(s=>s.name==='oneness; unity of religion');if(initial)history.replaceState({},'',url({subject:initial.id})+location.hash);}directory();const p=params(),subject=index.subjects.find(s=>s.id===p.get('subject'));
    ...
    const reading=readingPreferences(params(),savedReading);
    const rememberReading=next=>{
      try{localStorage.setItem('pi-reading',JSON.stringify(next));}catch{}
      history.replaceState({},'',url({mode:next.mode,size:Math.round(next.scale*100)})+location.hash);
      directory();
      for(const a of document.querySelectorAll('.subject-graph a, .summary-subject-link')){const u=new URL(a.href.baseVal||a.getAttribute('href'),location.href);u.searchParams.set('mode',next.mode);u.searchParams.set('size',Math.round(next.scale*100));a.setAttribute('href',u.pathname+u.search+u.hash);}
    };
    ...
    if(!review){
      ...
      if(status)status.textContent=`${data.selections.length.toLocaleString()} selections${printView?' · all passages':` · page ${page} of ${pages}`}`;
    }
