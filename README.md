...
Volume view (`volume-view.html`) provides a dropdown for all published PDFs, page navigation, zoom, extracted page text, and single/two-page layouts. Automatic uses two pages on screens at least 1100px wide. Volume, page, layout and zoom are shareable URL parameters. PDF.js 6.3.289 is bundled under `src/vendor/pdfjs/` with its license; no CDN or package install is needed.
