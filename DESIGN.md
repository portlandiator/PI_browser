...
Connected subjects is followed directly by the knowledge graph, without depth buttons, a toolbar, instructions, or subject/connection counts. Default is one hop, preserving existing `hops` deep links. Mouse/touch pan and zoom, keyboard arrows/+/-/0, and keyboard-accessible subject links remain available. The passage count shows total selections and pagination, omitting the source-group count.
