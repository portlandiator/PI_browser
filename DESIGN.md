...
Use the same reading-room typography and paper. Preserve every subject CSV row and its exact color; bright colors use a dark label background for contrast. Searchable subject directory, selected subject, a bounded neighborhood graph, ordinary related links, then paginated selections grouped by source. Imported/reviewed links use solid lines; suggestions use dashed lines and explicit labels. Graph navigation always has ordinary link equivalents. Selections preserve the imported wording separately from highlighted source paragraphs. Original paragraphs use the same number with an explicit unverified-correspondence notice. Uncertain matches belong in the editorial view. Editorial decisions are local drafts exported to a versioned file, with no account or runtime server.
