...
## Collection view
Compact persistent masthead with wordmark and Collection / About navigation. Main heading and search immediately follow. A large search input accepts words, quoted phrases, and filename IDs. Searches always include both languages. Results default to Date as recorded. Metadata filters occupy a left rail on desktop and a collapsible region on mobile. Result rows show source ID, descriptive title or incipit, metadata, and a contextual excerpt. Counts and pagination reflect the active filters. Missing values are explicit.
